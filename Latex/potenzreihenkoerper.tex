...
Nach Induktionsvoraussetzung sind alle $b_0, ..., b_{n-1}$ existieren und bestimmt. Für den n-ten Koeffizienten ergibt sich $0 =  a_0b_n + a_1b_{n-1} + ... + a_{n-1}b_1 + a_nb_0$. Da $ a_0$ ungleich $0$ ist, existiert die Lösung für $ b_n $. Sie ist eindeutig bestimmt durch $b_n = -\frac{1}{a_0} \sum_{j+k = n, ~j>0~}^{} a_jb_k$. Damit existiert ein eindeutiges Inverses zu $f$ und $f$ ist eine Einheit. \\
...
\end{satz}
\beweis{
Sei $f\in K((z))$ eine formale Laurentreihe, die obige Bedingung erfüllt. Wir können $f$ nach der Definition einer formalen Laurentreihe schreiben als $f = \sum_{n= - k}^{\infty}a_nz^n = g + h = \sum_{n=1}^{k}a_{-n}z^{-n} + \sum_{n=0}^{\infty}a_nz^n$. Wie in \ref{OrdnungNundZ} gezeigt, ist die Menge der ganzen Zahlen $\Z$ total geordnet. Der Hauptteil $h$ jeder formalen Laurentreihe besteht aus endlich vielen Summanden, was die Wohlordnung von supp($h$), dem Trägers des Hauptteils, impliziert (wir erinnern an \ref{TotalGeordnetEndlichIstWOhlgeordnet}). Der Träger des Nebenteils, supp($g$), ist aufgrund der Wohlordnung der natürlichen Zahlen wohlgeordnet. \\
...
\end{satz}
\beweis{Es genügt das Nachrechnen der Ringaxiome. Der Beweis verläuft analog zu \ref{RingFormalerPR}.
}
%
...
\beweis{Zunächst konstruieren wir formal die Inverse, wie wir es in Satz \ref{inverse Potenzreihe} bereits durchgeführt haben. Es bleibt zu zeigen, dass diese inverse Potenzreihe konvergiert. 
...
Sei
\[v(fg) = v( \sum_{n \in \Z}\sum_{n= m+k}a_mb_kz^n) \stackrel{\mathrm{!}}= v(f) + v(g),\] wobei $a_m = 0 \text{ für } m < n_0   \text{ und } b_k = 0$ für $k < m_0$. Betrachte $ n < n_0 + m_0 $. 
...
\item[zu B3']: Wenn $f, g$ wie oben definiert sind, erhalten wir für $v(f+g)$ die folgende Ungleichung: 
\begin{eqnarray*}
...
Nach Voraussetzung sind $ \text{supp}(f) = \lbrace g \in G \vert a_g 0\neq 0\rbrace \text{ und } \text{supp}(h) = \lbrace g \in G \vert b_g 0\neq 0\rbrace\ $  wohlgeordnet. Das kleinste Element von supp$(f+h)$ existiert und es gilt 
...
Der Träger der Summe $\sum_{g_1 + g_2 = g}^{}a_{g_1} b_{g_2}$ kann reduziert werden auf die Elemente $g = g_1 +g_2$ für die gilt, dass $g_1\in$ supp$(f)$ und $g_2\in$ supp$(h)$ liegt. Andernfalls wäre $a_{g_1}$ oder $b_{g_2}$ gleich null und das Produkt somit ebenso null. Da $G$ eine angeordnete abelsche Gruppe ist und $g_1, g_2 \in G$ sind, ist aufgrund der Abgeschlossenheit der Addition $g = g_1 + g_2 \in G$. Die Existenz der Summe ist nachgewiesen. Die Summe ist ungleich null für alle $g_1\in$ supp$(f)$ und $g_2\in$ supp$(h)$.\\
...
Sei $f\in K\left(\left(z^{G}\right)\right)$ mit $\min\left(\text{supp}\left(f\right)\right) > 0$ und $f=\sum_{g \in G}^{\infty}a_g z^g$, dann liegt für beliebige Körperelemente $\lambda_n$ die unendliche Reihe
\begin{eqnarray*}
h &=& \sum_{n=1}^{\infty}\lambda_nf^n \\
&=& \sum_{n=1}^{\infty}\lambda_n{\left(\sum_{g \in G}^{\infty}a_g z^g\right)}^n
\end{eqnarray*}
...
%
\begin{bem}
Sei $1_K + f \in \Upsilon$, und $\overline{\mathfrak{1_K +\sum_{n=1}^{\infty}(-f)^n}}$ das Inverse. Der Träger supp$(f)$ ist größer null und somit existiert das Inverse $1_K +\sum_{n=1}^{\infty}(-f)^n$ zu $1_K + f$.  Es gelten folgende Äquivalenzen
\begin{eqnarray*}
