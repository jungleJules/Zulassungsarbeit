...
\end{bem}
\beweis{Das neutrale Element der Addition $0_\C$ und das neutrale Element der Multiplikation $1_\C$ liegen trivialerweise in $\C\lbrace z \rbrace$. Es reicht, für $\C\lbrace z \rbrace$ zu beweisen, dass die Summe und das Produkt zweier konvergenter Potenzreihen wieder konvergent ist. \\
...
\]
konvergent ist. Aus dieser Gleichheit folgt, dass wir die Beschränktheit der Koeffizientenfolge $ (a_n)_{n \in \N_0}$ annehmen können. Nun bestimmen wir das Inverse ähnlich wie in \ref{inverse Potenzreihe} und geben eine positive untere Schranke an den Konvergenzradius an, woraus die Konvergenz der inversen Potenzreihe folgt.
...
Nach Definition der Multiplikation in $K((z))$ erhalten wir für den Koeffizienten des Produkts $gf$ bei $z^l$ somit: 
\begin{eqnarray*}
...
%
%
Nun versuchen wir auf dem Körper der Laurentreihen eine Bewertung zu finden.
...
&=& z^\gamma ~ a_\gamma \left(1_K + ~\sum_{g \in G \setminus \lbrace \gamma \rbrace} \frac{a_{g}}{{a_\gamma}}z^{g-\gamma}\right) \\
&=& z^\gamma ~ a_\gamma \left(1_K + ~\sum_{g \in G \setminus \lbrace \gamma \rbrace} b_{g-\gamma} z^{g-\gamma}\right),
\end{eqnarray*}
%
%
mit $b_{g-\gamma} = \frac{a_{g}}{{a_\gamma}}$. \\
...
Wir betrachten nun die archimedischen Klassen der angeordneten abelschen Gruppe $G$ und bezeichnen mit $\lbrack u_i\rbrack$ die archimedische Klasse von $u_i \in G$. Weil jedes $g\in U$ positiv ist und weil  jedes $u_i$ eine endliche Summe von Elementen aus $U$ ist, können wir Lemma \ref{maximumKlasseIstGanzeKlasse} anwenden und erhalten, dass in der angeordneten abelschen Gruppe $G$
\[ \lbrack u_i\rbrack = \lbrack \max_k\left( {g_i}_k\right)\rbrack \]
gilt. \\
%
%
%
Wir erhalten also, dass $\lbrack u_i\rbrack = \lbrack {g_i}^*\rbrack$ für $i=1, 2,...$ ist und $\lbrack g^*\rbrack = \min_{i\in \N}\left(\lbrack u_i\rbrack\right)$. 
Die Folge $\left(u_i\right)_{i\in\N }$ ist streng monoton fallend und wir wissen, dass es unter den Elementen ${g_i}_k$ aufgrund der Wohlordnung von $U$ ein kleinstes gibt. Daher gibt es ein $n\in \N$, sodass $\lbrack u_n\rbrack
= \lbrack u_{n+1}\rbrack = \lbrack u_{n+m}\rbrack = \lbrack g^*\rbrack$ für alle $m\in \N$ ist. \\
Das bedeutet also wir können jeder streng monoton fallenden Folge $\left({u'}_i\right)_{i\in\N}$ in $\mathfrak{U}$ ein Element ${g^*}'$ aus $U$ zuordnen, sodass ab einem bestimmten Index $i_0$, die archimedische Klasse von ${g^*}'$ der archimedischen Klasse von ${u'}_i$ entspricht.
Da $U$ nach Voraussetzung wohlgeordnet ist, gibt es unter den diesen Folgen zugeordneten Elementen ein kleinstes Element. \\
Wir nehmen an für unsere Folge $\left(u_i\right)_{i\in\N}$ ist $g^*$ das kleinste Element unter den ${g^*}'$.\\
...
%\[\lbrack u_1 \rbrack \supseteq \lbrack u_2 \rbrack \supseteq ... \supseteq \lbrack u_i \rbrack \supseteq ....\]
...
%
%
Jeder streng monoton fallenden Folge $\left({u'}_i\right)_{i\in\N}$ mit $\lbrack \mathfrak{g}\rbrack = \lbrack u_i \rbrack$ %u'_k ??????
...
Diese Folge hat die selbe Form wie \ref{eq: folgefürinvers}. Wir erhalten mit der gleichen Argumentation, dass $\lbrack {v'}_i\rbrack = \lbrack g^*\rbrack$ ist.
%
%
%
%
...
% 
%
\beweis{Wir zeigen zunächst die Wohldefiniertheit der unendlichen Reihe.\\
Für $\sum_{g \in G}^{}a_g z^g =: f \in K\left(\left(z^{G}\right)\right)$ mit $\min\left(\textup{supp}\left(f\right)\right) > 0$ kann  $\sum_{n=1}^{\infty}\lambda_n{\left(\sum_{g \in G}^{}a_g z^g\right)}^n$ für $\lambda_n \in K$ definiert werden, wenn für ein $\gamma$ der Koeffizient von $z^{\gamma}$ in $f^n$ nur für endlich viele $n\in \N$ ungleich null ist. Wir bezeichnen diesen Koeffizienten von $f^n$ bei $z^\gamma$ mit ${\left(f^n\right)}_\gamma$.\\
%$f^n$  ${\left(f^n\right)}_{\gamma} := {\left(\sum_{g \in G}^{}a_g z^g\right)}^n$, wobei $a_g=0$ für alle $g<\gamma$ ist, nur für endlich viele $n\in \N$ ungleich null ist.\\ %oder ist das g\neq \gamma??? also is es die summe über g = \gamma oder g\geq \gamma???
%
%
%
...
%
Die Folge ${\left(\gamma_{i1}\right)}_{i\in\N}$ enthält eine monoton wachsende Teilfolge, da $U$ wohlgeordnet ist. \\
%
%
Wir nehmen an, dass ${\left(\gamma_{i1}\right)}_{i\in\N}$ monoton wachsend ist. Die durch ${\gamma'}_i = -{\gamma_{i1}} +\gamma$ bestimmte Folge ist monoton fallend und weil $\mathfrak{U}$ wohlgeordnet ist und keine streng monoton fallende Folge existiert, ist ${\left({\gamma'}_i\right)}_{i\in\N}$ ab einem bestimmten Index konstant. Es gibt also ein $j\in\N$ mit ${\gamma'}_{j+m} = {\gamma'}_j = \gamma'$ für alle $m\in\N$. Das Element $\gamma'$ liegt somit in $\mathfrak{U}$ und daher in unendlich vielen der Mengen $nU$, $n\in\N$.\\
%
%
...
Die unendliche Reihe ist somit wohldefiniert.\\
Es bleibt zu zeigen, dass die unendliche Reihe in $K\left(\left(z^G\right)\right)$ liegt. Ihr Träger ist eine Teilmenge von $\mathfrak{U}$. Nach Lemma \ref{VereinigungWohlgeordnet} ist $\textup{supp}\left(\sum_{n=1}^{\infty}\lambda_nf^n\right)$ wohlgeordnet. Damit ist \[\sum_{n=1}^{\infty}\lambda_n{\left(\sum_{g \in G}^{}a_g z^g\right)}^n \in K\left(\left(z^G\right)\right).\]
}
%
%
%
...
\begin{eqnarray*}
f\cdot k 
&=& \left(\lambda\cdot z^g\left(1_K + h\right)\right) \cdot \left( \left(1_K + \overline{h}\right)z^{-g}\lambda^{-1}\right) \\
