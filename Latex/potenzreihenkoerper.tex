...
\section{Der Potenzreihenring}\label{potenzreihenring}
...
\end{satz}
\beweis{$\grqq\Leftarrow\grqq \text{ Beweis über Induktion:} \\
\text{Sei } a_0 \neq 0 \text{ dann folgt } \sum_{j=0}^{\infty}a_jz^j \sum_{k=0}^{\infty}b_kz^k = \sum_{n=0}^{\infty}\sum_{j+k=n}\left(a_jb_k\right)z^n\stackrel{\mathrm{!}}=1.$ Wir müssen nun eine entsprechende Potenzreihe $ \sum_{k=0}^{\infty}b_mz^m$ finden. \\ 
Für $b_0$ muss folgende Gleichung erfüllt sein: $a_0b_0= 1$. Da $a_0$ ungleich null ist besitzt die Gleichung eine eindeutige Lösung: $ b_0 = a_0^{-1}.$ \\
Angenommen es existiert ein $ b_k$ mit $ k < n$, sodass alle $c_m := a_j b_k$, für $1\le m < n$, gleich $0$ sind. Für den n-ten Koeffizienten ergibt sich $0 = c_m = a_0b_n + a_1b_{n-1} + ... + a_{n-1}b_1 + a_nb_0$. Bis auf $b_n$ sind alle Werte festgelegt. Da $ a_0$ ungleich $0$ ist, ist die Lösung für $ b_n $ eindeutig. \\
$\grqq\Rightarrow\grqq $ Es gilt $\sum_{j=0}^{\infty}a_jz^j \sum_{k=0}^{\infty}b_kz^k = 1. $ \\
Nach der Voraussetzung folgt $\sum_{j+k=n}a_jb_k = 0 $ für $n > 0$.  Daraus folgt unmittelbar $ a_0b_0 = 1 $ und somit muss $a_0$ ungleich $0$ sein.}
%
%
...
\end{equation}
Die Koeffizienten $b_n$ werden rekursiv durch diese Gleichungen definiert und die so entstandene Potenzreihe ist die Inverse. 
%
...
Seien $ f(z) := \sum_{n=0}^\infty  a_n z^n \text{ und } g(z) := \sum_{n=0}^\infty  b_n z^n$ mit 
\begin{center}
$ f(z) \cdot g(z) =  \sum a_nz^n \cdot \sum b_nz^n = 0$.
\end{center} 
Nach Definition der Multiplikation gilt also: $\sum_{j+k=n}a_jb_k = 0$, für alle $n \in \N $
Sei nun o.B.d.A. $\sum a_nz^n \neq 0$. Wir zeigen, dass die Potenzreihe $\sum b_nz^n$ gleich null ist. Es soll also kein Index n existieren, für den $b_n = 0$ ist und somit für 
$b_0, b_1,... ,b_{n-1} = 0$ folgt $b_n=0,$, für alle $n \in \N$.\\ 
...
Sei $\C\langle z \rangle$ die Menge der konvergenten Potenzreihen über dem Körper der komplexen Zahlen $\C$. $\C\langle z \rangle$ ist ein Unterring des Rings der formalen Potenzreihen $\C[[z]$. 
\end{satz}
\beweis{Wir haben bereits in \ref{intring} gezeigt, dass $\C[[z]$ ein Integritätsring ist. Nun bleibt für $\C\langle z \rangle$ noch zu beweisen, dass die Summe und das Produkt zweier konvergenter Potenzreihen wieder konvergent ist. \\
...
\begin{satz}\label{quotkoerper} %\cite{Lueneburg08}
...
\[\left(r,u\right)\left(s,v\right) := \left(rs, uv\right)\text{ definiert.}\] %TODO 
...
\end{satz}
\beweis{Es genügt, die Körperaxiome für (Quot(R), +, $\cdot$) nachzurechnen.
...
Wir nehmen an, dass die Schranke $a$ der Koeffizientenfolge $a_n$ größer 1 ist und es sei ohne Einschränkung $a_0 = 1$. Wir betrachten die Koeffizientenfolge $b_n$ des Inversen wie in \ref{eq: invers}. Es gilt:
\[
...
|b_n| \le (aC)^n
\]
Nach Konstruktion des Inversen \ref{eq: invers} ist die Ungleichung für $b_0$ erfüllt. Gelte die Abschätzung für $b_n$. Wähle $ C:= \frac{a}{a-1}$. Dann erhalten wir: \\
