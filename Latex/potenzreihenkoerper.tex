...
Das \textit{neutrale Element der Addition} $0_K$ ist die Nullreihe $ g(z) := \sum_{n=0}^\infty  b_n z^n$, wobei $b_n= 0 \text{ für alle n } \in \N_0 $. 
...
\end{satz}
\beweis{$``\Leftarrow "$
Sei $f = \sum\limits_{n=0}^{\infty}a_nz^n $ und es gelte $a_0 \neq 0$. Wir wollen zeigen, dass es eine Potenzreihe $g = \sum_{k=0}^{\infty}b_kz^k$ gibt, sodass das $f\cdot g = 1_K$ ist. Wir müssen nun eine entsprechende Potenzreihe $g$ finden, sodass 
\[f\cdot g= \sum_{j=0}^{\infty}a_jz^j \sum_{k=0}^{\infty}b_kz^k = \sum_{n=0}^{\infty}\sum_{j+k=n}\left(a_jb_k\right)z^n\stackrel{\mathrm{!}}=1\] ist. \\
Der Beweis erfolgt durch Induktion.  \\ 
Sei $n=0$, dann ist $\sum_{n=0}\sum_{j+k=n}\left(a_jb_k\right)z^n = \sum\limits_{j+k=0}\left(a_jb_k\right)z^0 = a_0b_01_K = a_0b_0$.
...
Nach Induktionsvoraussetzung existieren alle $b_0, ..., b_{n-1}$ und sind bereits bestimmt. Damit zeigen wir die Induktionsbehauptung, dass $b_n$ existiert und eindeutig definiert ist.\\
...
$``\Rightarrow "$ Es gilt $\left(\sum_{j=0}^{\infty}a_jz^j\right) \left(\sum_{k=0}^{\infty}b_kz^k \right)= 1. $ \\
Nach Definition \ref{definitionEinheitNullteiler} folgt, dass die $\sum_{j+k=n}a_jb_k = 0 $ sein muss für $n > 0$ und damit $ a_0b_0 = 1 $ gelten muss. Somit muss $a_0$ ungleich $0$ sein, damit $b_0$ eindeutig bestimmt ist.}
%
%
...
\end{align}
Auf der Menge $D$ der Punkte $z_0 \in \C$, für die $\sum_{n=0}^{\infty}a_n{z_0}^n$ konvergiert, wird somit eine Abbildung $z_0 \mapsto \sum_{n=0}^{\infty}a_n{z_0}^n $ definiert. Wir nennen $D$ den \textit{Konvergenzbereich}. Wir bezeichnen die größte Zahl $r \in \R_{geq 0}\cup \infty$ für welche die Potenzreihe für alle $z_0 \in \C$ mit $|z_0| < r$ konvergiert als \textit{Konvergenzradius}. Mit $D(0, r)$ bezeichnen wir die Menge aller Punkte in der offenen Kugel um $0$ für die die Potenzreihe konvergiert.
\end{defn}
...
Sei $f \in K((z))$ eine formale Laurentreihe. Dann ist der Träger der Laurentreihe ist wohlgeordnet.
\end{satz}
\beweis{
...
\end{satz}
\beweis{Es genügt das Nachrechnen der Ringaxiome. Das neutrale Element der Addition ist die Nullreihe $\sum_{n=k}^{\infty} a_kz^k$ mit $a_n = 0$ für alle $n \geq k$. Das neutrale Element der Multiplikation ist die Einsreihe $\sum_{n=k}^{\infty}a_kz^k$ mit $a_0 = 1$ und $a_n = 0$ für alle $0 \neq n \geq k$. Der Beweis der verbleibenden Bedingungen verläuft ähnlich zu \ref{RingFormalerPR}.
}
%
...
Nach Definition der Multiplikation in $K((z))$ erhalten wir somit: 
\begin{eqnarray*}
...
\end{cases}
\end{eqnarray}
Die Abbildung ist wohldefiniert im Bezug auf $m$, dem kleinsten Index für den $a_m\neq0$ gilt. Denn ist $m\geq 0$, so entspricht der Hauptteil der Laurentreihe der Nullreihe und die Reihe liegt somit in $K[[z]]$. Für den Fall $m \le 0$ existiert ein eindeutiges $g \in K[[z]]\subseteq \textup{Quot}K[[z]]$ und wir können $\sum_{n=m}^{\infty}a_nz^n = z^{-m}g$ schreiben.\\
...
Die Reihe $\sum_{n = l}^{\infty} b_nz^n $ kann deswegen in die Gestalt $z^l\sum_{n = 0}^{\infty} c_nz^n $, $c_0 \neq 0$ gebracht werden. Nach Satz \ref{potenzreihenringEinheit} kann diese Reihe $\sum_{n = 0}^{\infty} c_nz^n $ invertiert werden und wir erhalten
\begin{eqnarray*}
...
Die Folge ${(|a_n||{z_0}^n|)}_{n\in\N_0} $ ist somit eine Nullfolge und daher beschränkt. Nach dem Lemma von Abel \cite[S. 46]{ebeling2013funktionentheorie}konvergiert nun die Reihe $\sum_{n= 0}^{\infty} a_n{\zeta}^n$ für $\zeta \in\C$, $|\zeta| < |z_0|$ auf $D\left(0, |z_0|\right)$. Wähle $q = \frac{\zeta}{z_0}$. Wir erhalten, dass
\[
\overline{f} = \sum_{n=0}^{\infty}a_n{z_0}^n \left({\frac{\zeta}{z_0}}\right)^n = \sum_{n=0}^{\infty}a_n\zeta^n = f
\]
...
\]
Wir zeigen, dass |$b_n$|  durch ein Vielfaches von $a^n$ beschränkt ist. Ist dies gezeigt,  können wir eine positive untere Schranke des Konvergenzradius angeben.  Wir wählen $C \in R$ mit $ 1<C> \frac{a}{a-1}$. Mithilfe von Induktion beweisen wir, dass   
\[
|b_n| \le (aC)^n
\]
ist. 
...
Die Abbildung  $v\colon K((z))\rightarrow \Z\cup \lbrace \infty \rbrace$ definiert durch $v(f)= \min\left(\textup{supp}(f)\right)$ ist eine diskrete Bewertung wobei wir $v(\varnothing) = \infty$ setzen.
\end{satz}
%
\beweis{
%TODO DIe Surjektivität bracuh ich doch??
Klar: Die Abbildung ist surjektiv, da es zu jeder ganzen Zahl eine Laurentreihe mit diesem Startwert gibt und damit das Bild $v(f) = \min(\textup{supp}(f))$ ist. 
Nach Definition \ref{bewKoerper} sind für den Körper $K((z))$ und die angeordnete abelsche Gruppe $\Z$ noch B1'-B3' nachzuweisen. 
\begin{description}
...
\[v(fg) = v( \sum_{n \in \Z}\sum_{m+k=n}a_mb_kz^n) \stackrel{\mathrm{!}}= v(f) + v(g),\] ist, wobei $a_m = 0 \text{ für } m < n_0   \text{ und } b_k = 0$ für $k < m_0$ gilt. Wir betrachten $ n < n_0 + m_0 $. 
...
\beweis{
%
Der Ring der formalen Potenzreihen $K[[z]]$ ist der Bewertungsring der Bewertung $v\colon K((z))\rightarrow \Z\cup \lbrace \infty \rbrace$. Denn nach Definition des Bewertungsrings besteht dieser aus den Elementen des Körpers $K$ deren Bewertung größer gleich dem Nullelement der Gruppe ist. Die Elemente in $K((z))$ mit $v(a) \geq 0_\Z$ sind genau die Elemente aus $K[[z]]\subseteq K((z))$. Da $K((z)) = \textup{Quot}(K[[z]])$ ist, gilt die Behauptung.
%
%
...
%
%
\beweis{Wir untergliedern den Beweis und zeigen zunächst, dass für alle $g\in G$ die Summe $\sum_{g_1 + g_2 = g}^{}a_{g_1} b_{g_2}$ existiert und endlich ist.
...
In den Beweis der Distributivgesetze fließen die im Körper $K$ gültige Distributivität, die Rechengesetze für die Unbestimmte \ref{Potenzgesetze}, beziehungsweise die Kommutativität der angeordneten abelschen Gruppe $G$ mit ein.  
}
%
...
Es gilt weiterhin $g-\gamma > 0$ für alle $g\in \textup{supp}(f)\setminus \lbrace \gamma\rbrace$, da $\gamma = $min(supp$(f))$.
...
%
\begin{bem}
Sei $1_K + f \in \Upsilon$, dann ist $1_K +f$ invertierbar mit dem Inversen $\sum_{n=0}^{\infty}\left(-f\right)^n$.
...
Somit lässt sich $K\left(\left(z^G\right)\right)$ einbetten in $K\left(\left(z^\R\right)\right)$.
\end{bsp}
%
%
% 
...
%
\begin{satz}
Die Abbildung $v\colon K\left(\left(z^G\right)\right) \to G\cup \lbrace\infty\rbrace,~ f \mapsto \min(\textup{supp}(f))$ mit $v(\varnothing)=\infty$  ist eine Bewertung auf $K\left(\left(z^G\right)\right)$.
\end{satz}
...
Sei nun $g = g_1 + g_2$. Das Produkt $ a_{g_1}b_{g_2}$ ist ungleich Null und daher erhalten wir, dass $v(fg) = g_1+g_2 = v(f) + v(h)$ ist.
