...
%
\beweis{
Sei  $f \in K\left(\left(z^{G}\right)\right)$ mit $f = \sum_{g \in G}^{\infty}a_g z^g$. Nach Definition einer formalen Potenzreihe ist supp$(f)$ wohlgeordnet und es existiert damit ein kleinstes Element $\gamma\in G$ mit $\gamma=$min(supp$(G))$. Da $G$ eine angeordnete abelsche Gruppe ist, existiert für jedes Element ein additives Inverses. Es gilt:
\begin{eqnarray*}
f&=& z^g ~\sum_{g \in G} a_{g} z^{g-\gamma}\\
...
wobei supp($~\sum_{g \in G\setminus \lbrace g \rbrace} \frac{a_{g}}{{a_\gamma}^{-1}} z^{g-\gamma}) \ge 0$ ist.
Das Inverse zu $a_\gamma$ existiert, da der Koeffizient im Körper $K$ liegt und da $a_\gamma \neq 0$ für $\gamma \in$supp$(f)$ ist. 
