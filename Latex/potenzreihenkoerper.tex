...
Daraus folgt die Beschränktheit der Folge ${(a_n|z_0|)}_{n\in\N_0} $. Da $g$ konvergiert, ist die Folge  beschränkt und für die inverse Potenzreihe $\overline{g} = \sum_{n=0}^{\infty}a_n\left(\frac{z}{z_0}\right)^n$  gilt: \\
\[\overline{g} = \sum_{n=0}^{\infty}a_n{z_0}^n \left({\frac{z}{z_0}}\right)^n = \sum_{n=0}^{\infty}a_nz^n =  f \text{ mit }\omega:= \frac{z}{z_0}\]
...
\]
Wir zeigen, dass |$b_n$| beschränkt ist durch ein Vielfaches von $a^n$. Deswegen können wir eine positive untere Schranke des Konvergenzradius angeben. Mithilfe von Induktion beweise wir, dass ein $C > 1$ existiert, mit $C \in \R $, sodass  
\[
|b_n| \le (aC)^n
\]
ist. 
Nach Konstruktion des Inversen \ref{inverse Potenzreihe} ist die Ungleichung für $b_0$ erfüllt. Die Abschätzung gelte für $b_n$. Wir wählen $ C> \frac{a}{a-1}$. Wir erhalten folgende Abschätzung für den Koeffizienten $b_{n+1}$:\\
