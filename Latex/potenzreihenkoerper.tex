...
Sei $f\in K\left(\left(z^{G}\right)\right)$ mit $\text{min}\left(\text{supp}\left(f\right)\right) > 0$ und $f=\sum_{g \in G}^{\infty}a_g z^g$, dann liegt für beliebige Körperelemente $\lambda_n$ die unendliche Reihe
\begin{eqnarray*}
...
\end{lemma}
\beweis{Die Reihe $h$ liegt im Potenzreihenring, wenn die Vereinigung der Träger supp($f^n$) wohlgeordnet ist in der Anordnung von G. Weiterhin dürfen für jedes Element des Trägers $a \in $ supp($f$) nur endliche viele ganze Zahlen n existieren, sodass der $a$-te mit n potenzierte Koeffizient ungleich null ist. \\
