...
\end{bem}
\beweis{Das neutrale Element der Addition $0_\C$ und das neutrale Element der Multiplikation $1_\C$ sind trivialerweise konvergent und liegen in $\C\lbrace z \rbrace$. Es reicht, für $\C\lbrace z \rbrace$ zu beweisen, dass die Summe und das Produkt zweier konvergenter Potenzreihen wieder konvergent ist. \\
...
%
%
Sei $\sum_{g \in G}^{}a_g z^g =: f \in K\left(\left(z^{G}\right)\right)$ mit $\min\left(\textup{supp}\left(f\right)\right) > 0$. Die \textbf{unendliche Reihe}
%
%
\begin{eqnarray*}
...
Für $\sum_{g \in G}^{}a_g z^g =: f \in K\left(\left(z^{G}\right)\right)$ mit $\min\left(\textup{supp}\left(f\right)\right) > 0$ kann  $\sum_{n=1}^{\infty}\lambda_n{\left(\sum_{g \in G}^{}a_g z^g\right)}^n$ für $\lambda_n \in K$ definiert werden, wenn für ein $\gamma \in \textup{supp}(f)$ der Koeffizient von $z^{\gamma}$ in $f^n$ nur für endlich viele $n\in \N$ ungleich null ist. Wir bezeichnen diesen Koeffizienten von $f^n$ bei $z^\gamma$ mit ${\left(f^n\right)}_\gamma$.\\
