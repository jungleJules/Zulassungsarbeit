...
%
\begin{bem}
Die Multiplikation formaler Laurentreihen ist unter der Bedingung wohldefiniert, dass formale Laurentreihen höchstens endliche viele Terme mit negativen Exponenten besitzen. Diese Bedingung ist gleichbedeutend mit der Forderung nach der Wohldefiniertheit des Trägers einer formalen Laurentreihe. \\
 Diese Bedingung ist unverzichtbar zur Wohldefiniertheit der Multiplikation. 
\end{bem}

\begin{satz}
Sei $f \in K((z))$ eine formale Laurentreihe mit endlichem Hauptteil. Dann gilt, der Träger der Laurentreihe ist wohldefiniert.
\end{satz}
\beweis{
...
Für supp$(f)$ gilt trivialierweise, dass er der Vereinigung von supp($g$) und supp($h$) entspricht. Unter Verwendung von Satz \ref{wohlgeordnvereinigung} erhalten wir die Wohlordnung des Trägers der formalen Laurentreihe. 
}
%
%
%
...
%
%
Im Folgenden beschränken wir uns aufgrund der Konvergenzbetrachtung wieder auf den Körper $\C$. 
%
%Nach Kaiser Skript
\begin{defn}
...
\]
Wir zeigen, dass |$b_n$| beschränkt ist durch ein Vielfaches von $a^n$. Ist dies gezeigt,  können wir eine positive untere Schranke des Konvergenzradius angeben. Mithilfe von Induktion beweise wir, dass ein $C > 1$ existiert, mit $C \in \R $, sodass  
\[
|b_n| \le (aC)^n
\]
ist. 
Nach Konstruktion des Inversen \ref{inverse Potenzreihe} ist die Ungleichung für $b_0$ erfüllt. \\
Die Abschätzung gelte für $b_n$. Wir wählen $ C> \frac{a}{a-1}$. Wir erhalten folgende Abschätzung für den Koeffizienten $b_{n+1}$:\\
...
Dazu betrachten wir zunächst den Träger \ref{traeger} der Laurentreihe supp$(f) := \lbrace n \in \Z | a_n \neq 0 \rbrace$. Nach \ref{bewKoerper} suchen wir einen surjektiven Gruppenhomomorphismus (nach B3' \ref{bewKoerper}).
...
Nach Definition \ref{bewKoerper} sind noch (B1'-B3') nachzuweisen mit ($\Z \cup \lbrace \infty\rbrace$, +) als Bildmenge. 
\begin{enumerate}
