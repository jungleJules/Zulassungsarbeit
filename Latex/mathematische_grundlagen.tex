...
\[ \frac{r}{u} = \frac{r_1}{u_1} \text{ genau dann, wenn } ru_1 = ur_1
\]
...
Das Distributivgesetz kann mithilfe der Bruchrechenregeln leicht gezeigt werden. Somit ist der Körper (Quot(R), +, $\cdot$) konstruiert.
...
\[ a\leq b \text{ ist, genau dann, wenn } b -a \in \N_0 \text{ oder } a = b \text{ gilt.}
\]
...
\end{enumerate}
Genügt eine Teilmenge $P = \lbrace x \in G | x \geq 1\rbrace$ einer Gruppe $G$ den Bedingungen P1- P3, so nennt man $\left(G,\circ\right)$ \textit{anordnungsfähig}.
\end{bem}
%
%
%
...
In der daraus entstandenen Folge ist jedes Element größer als $a_{i1}$. Sie enthält wiederum keine streng monoton fallende oder konstante Teilfolge. In der so entstandenen Folge ist jedes enthaltene Element echt größer als $a_{i1}$. \\
...
\end{folg}
\beweis{Angenommen es gäbe unendlich viele paarweise verschiedene $\left(v_n, w_n\right) \in V\times W$ für $n \in \N$, sodass $g = v_n + w_n$. Da $V$ und $W$ wohlgeordnet sind, besitzt weder $\left(v_n\right)_{n\in\N}$ noch $\left(w_n\right)_{n\in\N}$ eine streng monoton fallende Teilfolge. Die Folge $\left(v_n + w_n\right)_{n \in \N}$ enthält keine konstante Teilfolge, da sie nach Voraussetzung paarweise verschiedene Glieder besitzt. Nach Satz \ref{unendlicheFolgeEigenschaften} hat $\left(v_n + w_n\right)_{n \in \N}$ eine streng monoton steigende Teilfolge. Widerspruch, da $g$ konstant bleiben muss.
}
%
%
%
...
Es gilt die \textit{Dreiecksungleichung} für alle $a, b \in G$:
...
\item[(ii)] $S_g$ ist nach oben beschränkt für alle $g \in P$. Die Abbildung $\Phi: P \rightarrow \R^{+}, g \mapsto$ sup$\lbrace S_g \rbrace$ ist somit wohldefiniert.  
...
Seien $r, s \in \Q_{>0} \text{ mit } r < s \text{ und } s \in S_g \text { mit } r := \frac{k}{l} \text{ und } s:= \frac{m}{n}, \text{ wobei } m, n ,k, l \in \N$. Nach Voraussetzung gilt $kn < lm $ und da $s \in S_g: m\alpha \le ng$ und $kn\alpha \le lm\alpha \le lng.$ Daraus wiederum folgt: $\frac{kn}{ln} = r \in S_g$.\\
%
%
% 
%
...
Sei $G$ eine angeordnete abelsche Gruppe, $U$ eine Untergruppe und $g \in G$. 
...
%
%
%TODO: fehlt argument warum C keine konvexe UGR von C/D!
Zu S2: Nach dem Korrespondenzsatz \cite{fischer08} entsprechen die Untergruppen von $C/D$ bijektiv den Untergruppen von $C$, die $D$ enthalten. Die Ordnung auf $C/D$ ist die induzierte Ordnung. Damit überträgt der Isomorphismus ebenfalls die Konvexitätseigenschaft. 
Angenommen die Faktorgruppe $C/D$ enthält nicht nur die trivialen konvexen Untergruppen $0_{C/D}$ und $C/D$. Dann gibt es eine konvexe Untergruppe $H$ in $C/D$ mit $D \subset H \subset C/D$. Auf Grund des Isomorphismus gibt eine konvexe Untergruppe $H'$ in $G$ mit $D \subset H' \subset C$. Da $C, D$ ein Sprung ist, gibt es aber zwischen $C$ und $D$ keine weiteren Untergruppen. Dies ist ein Widerspruch zur Annahme. $D$ ist das neutrale Element der Faktorgruppe $C/D$.

%
%
%In $\Sigma$ existiert keine Untergruppe zwischen $C$ und $D$. Da $C, D$ ein Sprung ist, gibt es aber zwischen $C$ und $D$ keine weiteren Untergruppen. $D$ ist das neutrale Element der Faktorgruppe $C/D$. \\
%
%
%
...
Sei $\left(G, +\right)$ eine angeordnete Gruppe. Wir bezeichnen die archimedische Klasse, in der das Element $a\in G$ liegt  mit $[a]$. Die Gesamtheit der archimedischen Klassen von $G$ nennen wir $[G]$. Die Abbildung $G \to [G], a \mapsto [a]$, ist eine Bewertung (dabei ist $[0]$ das kleinste Element) und wird als \textit{natürliche Bewertung} bezeichnet. %\cite{priesscrampe83}
%
%
%
%
