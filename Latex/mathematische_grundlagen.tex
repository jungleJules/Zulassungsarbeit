...
Wir setzen \textup{Quot}$(R) = \left(R\times R\setminus\lbrace 0\rbrace , \sim\right)$ und bezeichnen die Äquivalenzklasse von $(r,u) $ mit $\frac{r}{u}$. Nach Definition gilt für $\left(r_1, u_1\right), \left(r_2, u_2\right) \in \left(R\times R\setminus\lbrace 0\rbrace , \sim\right)$:
...
%
%
Wir wollen zeigen, dass $\frac{r_1}{u_1} +\frac{s_1}{v_1} = \frac{r_2}{u_2}+\frac{s_2}{v_2}$ ist. Wir erhalten
%
\begin{eqnarray*}
...
&=& \left(\varphi(a)\varphi(d) + \varphi(b)\varphi(d)\right){\left(\varphi(b)\varphi(d)\right)}^{-1}\\
&=& \left(\varphi(a)\varphi(d) + \varphi(b)\varphi(d)\right){\left(\varphi(b)\right)}^{-1}{\left(\varphi(d)\right)}^{-1} \\
...
&=&\left(\varphi(a)\varphi(d)\right){\left(\varphi(b)\varphi(d)\right)}^{-1}\\
&=& \varphi(a){\varphi(b)}^{-1} \varphi(c){\varphi(d)}^{-1} \\
&=& \Phi\left(\frac{a}{b}\right) \Phi\left(\frac{c}{d}\right).
\end{eqnarray*}
Weiter gilt $\Phi(1_R) = \Phi\left(\frac{1_R}{1_R}\right) = \varphi(1_R){\varphi(1)}^{-1} = 1_K$.\\
Sei $a\in R$, so erhalten wir $\Phi\left(\frac{a}{1}\right)=\varphi(a){\varphi(1)}^{-1} = \varphi(a)$.\\
...
In diesem Kapitel fassen wir jene Begriffe und Bezeichnungen zusammen, die zur Betrachtung des verallgemeinerten Potenzreihenkörpers benötigt werden. Nach einer Einführung in die Theorie angeordneter abelscher Gruppen beschäftigen wir uns mit deren Wohlordnung, eine Eigenschaft, die für die Konstruktion des Potenzreihenkörpers unabdingbar ist. Mithilfe der Archimedizität führen wir eine spezielle Art der Anordnung von Gruppen ein. Daran schließt die zentrale Aussage des Kapitels an: der Satz von Hölder. Der Satz besagt, dass archimedisch angeordnete Gruppen in die additive Gruppe von $\R$ eingebettet werden können. \\
Nachdem wir kurz auf die angeordnete Menge konvexer Untergruppen eingehen, geben wir einen kurzen Ausblick auf die Bewertungstheorie.
...
In der daraus entstandenen Folge ist jedes Element größer als $a_{i_1}$. Sie enthält wiederum keine streng monoton fallende oder konstante Teilfolge. In der so entstandenen Folge ist jedes enthaltene Element echt größer als $a_{i1}$. \\
