...
%
%
Die Menge $W_A$ ist somit unter endlicher Vereinigung und dem Schnitt über $n\in \N$ Elementen abgeschlossen.
...
\item[(ii)] $S_g$ ist nach oben beschränkt für alle $g \in G$. Die Abbildung $\Phi: P \rightarrow \R^{+}, g \mapsto$ sup$\lbrace S_g \rbrace$ ist somit wohldefiniert.  
\item[(iii)]Für alle $g, h \in P$ gilt $g \le h $ genau dann, wenn $ S_g \subseteq S_h$ ist. Dies gilt genau dann, wenn $\Phi(g) \le \Phi(h)$ erfüllt ist.  
...
Da $\Phi$ offensichtlich ordnungserhaltend nach (iii) ist, ist $\Phi$ ein ordnungserhaltender Isomorphismus.}
...
Die Ordnung auf $\Sigma$ ist total. Denn angenommen es gäbe $C,D \in \Sigma$ sodass weder $C \subseteq D$, noch $D \subseteq C$ noch $C= D$ gilt. Sei $c \in C$ und $d \in D$. Sei $0 < |d| <|c|$ dann gilt nach Definition der Konvexität, dass $d \in C$ liegt. Dies ist ein Widerspruch zur Voraussetzung.
}
%
...
\end{satz}
\beweis{
Zu S1: Seien $C, D \in \Sigma$. Nach Lemma \ref{untergruppeangeordnet} ist $\Sigma$ angeordnet.
...
Zu S2: In $\Sigma$ existiert keine Untergruppe zwischen $C$ und $D$. Nach dem Korrespondenzsatz \cite{fischer08} entsprechen die Untergruppen von $C/D$ bijektiv den Untergruppen von $C$, die $D$ enthalten. Die Ordnung auf $C/D$ ist die induzierte Ordnung. Damit überträgt der Isomorphismus ebenfalls die Konvexitätseigenschaft. Da $C, D$ ein Sprung ist, gibt es aber zwischen $C$ und $D$ keine weiteren Untergruppen. $D$ ist das neutrale Element der Faktorgruppe $C/D$. Daraus können wir schließen, dass die Faktorgruppe $C/D$ nur die trivialen konvexen Untergruppen $0_{C/D}$ und $C/D$ enthält. \\
Angenommen $C/D$ wäre nicht archimedisch geordnet. Dann  Sei $0_G \neq g \in G$, dann gibt es ein Element $h\in C/D$ sodass, $ng < h$ ist für $n = 0, \pm 1,\pm 2, ...$. Wie vorher gezeigt enthält $C/D$ nur die trivialen konvexen Untergruppen. Also muss $g = 0_{C/D}$ sein. Dies ist ein Widerspruch zur Annahme und wir sehen, dass die Faktorgruppe archimedisch angeordnet ist.
