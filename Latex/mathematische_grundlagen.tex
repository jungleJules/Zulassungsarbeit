...
\item[R2: ] Die Multiplikation $\cdot$ ist assoziativ: Für $a, b,c \in R$ gilt $a\cdot \left(b \cdot c\right) = \left(a \cdot b \right) \cdot c$. Das neutrale Element der Multiplikation wird mit $1_R$ bezeichnet.%Blöd weil ich Monoid brauche: $\left(R, \cdot\right)$ ist ein Monoid,
...
\end{satz}
\beweis{ Wir zeigen die Wohldefiniertheit der Verknüpfungen. Sei $\left({r_1},{u_1}\right), \left({r_2},{u_2}\right), \left({s_1},{v_1}\right), \left({s_2},{v_2}\right) \in R\times R\setminus\lbrace 0\rbrace$ und $\frac{r_1}{u_1}=\frac{r_2}{u_2}$ und $\frac{s_1}{v_1} = \frac{s_2}{v_2}$. Nach Definition gilt $r_1u_2 = u_1r_2$ und $s_1v_2 = v_1s_2$. Daher erhalten wir:
\[ u_1v_1\left(r_2v_2 + u_2s_2\right) = u_1v_1r_2v_2 + u_1v_1u_2s_2 = u_1r_2v_1v_2 + u_1u_2v_1s_2 = r_1u_2v_1v_2 + u_1u_2s_1v_2 = u_2v_2\left(r_1v_1 + u_1s_1\right)\]
Es gilt also $\frac{r_2v_2 + u_2s_2}{u_2v_2} = \frac{r_1v_1 + u_1s_1}{u_1v_1}$. Somit ist die Addition wohldefiniert.\\
...
Die Wohldefiniertheit der Multiplikation folgt analog. Denn für die oben definierten Äquivalenzklassen erhalten wir $ u_2v_2\left(r_1s_1\right) = u_2v_2r_1s_1 = r_1u_2s_1v_2 = u_1r_2v_1s_2 = u_1v_1\left(r_2s_2\right) $.\\
%\begin{equation*}
...
Weiter ist (Quot(R), $\cdot$) eine abelsche Gruppe, wobei $\frac{1}{1}$ das neutrale Element und $\left(\frac{r}{u}\right)^{-1} = \frac{b}{a}$ das inverse Element ist. \\
Das Distributivgesetz kann leicht nachgerechnet werden. Somit ist der Körper (Quot(R), +, $\cdot$) konstruiert.
...
\item[(ii)] Die Teilmenge $P = \lbrace x \in G | x \geq 0_G \rbrace$ einer Gruppe $G$ den folgenden Bedingungen:
\begin{enumerate}
...
%
Angenommen der Positivbereich $P$ hat ein kleinstes Element. Es gibt also ein $g \in P$ mit $0_G \le g$ wobei für jedes $h\in G$, mit $0_G \leq h < g$, folgt, dass $h = 0_G$ gilt. Wegen der archimedischen Eigenschaft gibt es zu jedem $h\in G$ eine ganze Zahl $n\in \Z$, sodass $ng \leq h < \left(n+1\right)g$ gilt. Daher erhalten wir $0_G\leq h + \left(-n\right)g < (n+1)g -ng = g$. Also gilt, dass $h + \left(-n\right)g = 0_G$ ist. Infolgedessen ist $G= \langle g\rangle$, wobei $\langle g\rangle$ die von $g$ erzeugte Untergruppe ist, o-isomorph zur Gruppe der ganzen Zahlen ist bezüglich der Zuordnung $ng \mapsto n$.\\
...
Für beliebige $m, n, p \in \N$ gilt die Äquivalenz $m\alpha \le n g \Leftrightarrow m p \alpha \le n p \alpha$. Die Darstellung von $r \in \Q_{>0}$ als Quotient zweier natürlicher Zahlen hängt also nicht damit zusammen, ob $ r$ in $S_g$ enthalten ist.\\
...
Für die negativen Gruppenelemente $g \in G$, mit $g < 0$  und daher $-g >0$ erhalten wir aufgrund der Homomorphismuseigenschaften $\frac{\phi(g)}{g} = \frac{-\cdot\phi(g)}{-\cdot g} = \frac{\phi(-g)}{-g} = \frac{\phi(g_1)}{g_1}$. Mit der positiven Konstanten $r  = \frac{\Phi(g_1)}{g_1}$ ist die Aussage $\phi(g) = r \cdot g$ für alle $g \in G$ gezeigt. 
}
%
%
%
...
\end{folg}
\beweis{
Wir verwenden den Satz von Hölder, damit ist $G$ o- isomorph zu einer Untergruppe der additiven Gruppe der reellen Zahlen. Nach Satz \ref{homomorphismus nach R} wissen wir bereits, dass es eine reelle positive Zahl $r$ gibt, sodass ein o-Monomorphismus $g \in G$ auf $rg$ abbildet. Die Konkatenation des o-Isomorphismus und o-Monomorphismus ergibt wiederum einen o-Monomorphismus. Für ein $g> 0$ mit $g\in G$, wobei $G$ existiert also ein $0 < r \in R$ mit $r= \frac{1}{g}$. Dieser Quotient ist definiert, da $g >0$ ist und $G$ o-isomorph zu einer Untergruppe des Körpers der reellen Zahlen ist, $r$ ist offensichtlich positiv, da $g$ nach Voraussetzung positiv ist. Die Abbildung $\Phi \colon G\to \R, g\mapsto rg = \frac{1}{g}g = 1$ ist nach Satz \ref{homomorphismus nach R} ein o-Monomorphismus. \\
Die Eindeutigkeit des o-Monomorphismus der Form $\Phi \colon G\to \R, g\mapsto 1$ folgt direkt aus der Eindeutigkeit des Quotienten $\frac{1}{g}$, welche aufgrund der o-Isomorphie zu einer Untergruppe der reellen Zahlen folgt.
}
%
%
%
...
$``\Leftarrow "$ Angenommen $G$ sei nicht archimedisch. Sei $0 < a <b$ mit $a,b \in G$, dann gilt für alle $n = 0, \pm 1,\pm 2, ...$ dass $na \leq b$ ist. Das heißt es gilt für alle $a\in G$ mit $a <b$ die Ungleichung $na \leq b$ und damit $na \in G$. Die von $a$ erzeugte konvexe Untergruppe $\langle a\rangle$ ist ungleich $G$ und wir erhalten somit $\langle a\rangle = 0_G$. Dies ist ein Widerspruch zur Annahme.
}
%
%
%
...
%
%
Da $C/D$ eine angeordnete Gruppe mit der induzierten Ordnung ist, erhalten wir nach Satz \ref{archimedischNur TrivialeUGR}, dass $C/D$ archimedisch angeordnet ist.\\ 
